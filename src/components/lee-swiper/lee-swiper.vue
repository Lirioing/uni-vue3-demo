  ...
  <view :class="cls">
    ...
    >
      <block v-for="(img, idx) in list" :key="idx">
        ...
        >
          <view
            :class="[`${prefixCls}-item-image`,{ zoom: zoom && innerCurrent !== idx }]"
            :style="{borderRadius: radius+'rpx'}">
            <image mode="aspectFill" :src="img" :style="{borderRadius: radius+'rpx'}" />
          </view>
        </swiper-item>
      </block>
    </swiper>
    <template v-if="indicator">
      ...
      >
        <block v-for="(_, idx) in list.length" :key="idx">
          <view
            class="dots"
            :style="{ backgroundColor: innerCurrent === idx ? indicatorActiveColor : indicatorColor }"
          ></view>
        </block>
      </view>
    </template>
